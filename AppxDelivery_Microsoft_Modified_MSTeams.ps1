...
function Remove-CMSLogFile
{
    ...
    [string]$logPath = ('{0}\Microsoft\IntuneManagementExtention\Logs\' -f $env:ProgramData)
...
function Start-CMSLogging
{
    ...
    [string]$loggingPath = ('{0}\Microsoft\IntuneManagementExtention\Logs\' -f $env:ProgramData)
...
if((-not ([string]::IsNullOrEmpty($DependencyFileName))) -and (-not ([string]::IsNullOrEmpty($LicenseFileName))))
{
...
if(-not ([string]::IsNullOrEmpty($LicenseFileName)))
{
    Install-AppxProvisionedPkg -AppName $AppName -PackageFileName $PackageFileName -LicenseFileName $LicenseFileName
}

if(([string]::IsNullOrEmpty($DependencyFileName)) -and ([string]::IsNullOrEmpty($LicenseFileName))) 
{
